...
A CLI + Streamlit tool that deep-scans a single web page and produces an SEO/AEO/GEO audit. A deterministic scanner extracts every audit-relevant signal (full content, schema, meta, robots.txt, sitemap) into Redis; then **three specialist ReAct agents** (SEO, AEO, GEO) each loop over that data (plus a local RAG handbook) and a synthesis step combines them into a composite score + executive summary. The split is the core idea: the scanner extracts facts exactly, the LLMs only reason.
...
- **Streamlit "Auto-Fix → PR (Claude Code)" section** — uses the last audit's findings: **Run Claude fix** (requires a clean working tree) → review the **git diff** (the approval gate) → **Approve → open PR** (or **Discard changes**).
