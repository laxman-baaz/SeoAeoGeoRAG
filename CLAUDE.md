...
0. **Full-site mode (fan-out)** — `crawler.py → crawl_site(start_url, max_pages=100, delay=0.3)` does a polite BFS (sitemap-seeded, same-domain links, respects robots.txt, per-page content trimmed) storing every page in Redis. The audit is `agent.run_site_audit_fanout(domain)`: a **hybrid** —
